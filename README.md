...
Server runs at `http://localhost:3456` (HTTP + WebSocket).
...
1. Go to `chrome://extensions/` / 打开 `chrome://extensions/`
2. Enable "Developer mode" / 开启「开发者模式」
3. Click "Load unpacked" → select `tiktok-live-extension/dist/chrome-mv3`
   点击「加载已解压的扩展程序」→ 选择 `tiktok-live-extension/dist/chrome-mv3`
...
4. Data starts flowing in real-time / 数据实时流入
5. Export anytime via JSON, Excel, or HTML report / 随时导出
